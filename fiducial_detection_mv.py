...
model = YOLO('fiducial_weights3.pt')
...
def loop():
    ...
    # Set manual exposure time to 30ms
    ...
    while(cv.waitKey(1) & 0xFF) != ord('q'):
        ...
        try:
            ...
            for data in prediction.boxes.data.tolist():
                ...
                cv.rectangle(frame, (xmin, ymin), (xmax, ymax), GREEN, thickness=2)
                ...
                cv.putText(frame, coordinates, (10,30), cv.FONT_HERSHEY_SIMPLEX, .5, GREEN, 2, cv.LINE_AA)
                cv.circle(frame, (center_x, center_y), radius=2, color=GREEN, thickness=-1)
